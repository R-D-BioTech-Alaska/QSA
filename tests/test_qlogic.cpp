...
int main() {
    ...
    const QExactLinearResult reordered_linear_result = reordered_linear.solve();
    require(reordered_linear_result.solution == solved.solution &&
            reordered_linear_result.canonical == solved.canonical,
    ...
    QExactLinearSystem capped_linear(
        {"x"},
        QMathType::scalar_type(QMathScalar::Rational),
        QExactLinearConfig{1U, 1U, 2U});
    capped_linear.add_equation(std::vector<QRational>{QRational(1)}, QRational(1));
    rejected = false;
    try {
        capped_linear.add_equation(std::vector<QRational>{QRational(1)}, QRational(2));
    } catch (const QMathError&) {
        rejected = true;
    }
    require(rejected && capped_linear.equation_count() == 1U,
            "exact linear equation cap did not fail closed");

    std::cout << "QHorn logic, mathematical fabric and exact linear-system tests passed\n";
}
