...
CausalParameterizedPlanHandle* as_causal_parameterized_plan(
...
CausalPauliPlanHandle* as_causal_pauli_plan(qcausal_pauli_plan_handle handle) {
    if (handle == nullptr) {
...
void invalidate_causal_qsc(CausalHandle& handle) {
    handle.qsc_cache_valid = false;
    handle.qsc_cache.clear();
}

const std::vector<std::uint8_t>& encoded_causal_qsc(CausalHandle& handle) {
...
int guarded_causal(Function&& function) {
    try {
        function();
...
qubit::Operation convert_causal_operation(
...
qubit::ParameterizedOperation convert_causal_parameterized_operation(
    const qstate_parameterized_operation& operation) {
    return qubit::ParameterizedOperation{
        convert_causal_operation(operation),
        operation.parameter_slot,
        operation.sample_slot,
    };
}

std::vector<std::string> decode_causal_pauli_words(
    std::size_t qubit_count,
    const char* words,
...
int qcausal_adopt(qcausal_handle target, qcausal_handle selected) {
    return guarded_causal([&] {
        ...
        invalidate_causal_qsc(*target_handle);
        invalidate_causal_qsc(*selected_handle);
    });
}
...
size_t qcausal_qsc_size(qcausal_handle handle) {
    try {
        const std::size_t value = encoded_causal_qsc(*as_causal(handle)).size();
        causal_last_error.clear();
        return value;
    } catch (const std::exception& error) {
        causal_last_error = error.what();
...
int qcausal_qsc_write(
    ...
    size_t output_size) {
    return guarded_causal([&] {
        const auto& encoded = encoded_causal_qsc(*as_causal(handle));
...
qcausal_parameterized_plan_handle qcausal_parameterized_plan_create(
    ...
    try {
        ...
        for (std::size_t index = 0; index < operation_count; ++index) {
            converted.push_back(
                convert_causal_parameterized_operation(operations[index]));
        }
...
size_t qcausal_parameterized_plan_parameter_count(
    ...
    try {
        const std::size_t value =
            as_causal_parameterized_plan(plan)->plan.parameter_count();
        causal_last_error.clear();
        return value;
    } catch (const std::exception& error) {
        causal_last_error = error.what();
...
int qcausal_parameterized_plan_execute(
    ...
    return guarded_causal([&] {
        ...
        as_causal_parameterized_plan(plan)->plan.execute(
            causal->state.write(),
...
qcausal_pauli_plan_handle qcausal_pauli_plan_create(
    ...
    try {
        std::vector<std::string> decoded =
            decode_causal_pauli_words(qubit_count, words, word_count);
...
size_t qcausal_pauli_plan_observable_count(qcausal_pauli_plan_handle plan) {
    try {
        const std::size_t value =
            as_causal_pauli_plan(plan)->plan.observable_count();
        causal_last_error.clear();
        return value;
    } catch (const std::exception& error) {
        causal_last_error = error.what();
